...
//  }
  ...
  public boolean saveServerSettingsFile(String settingsFile)
  {
    ...
    try
    {
      ...
      fileServerSettings.setProperty("vate.server.authentication.type", tlsAuthentication ? "TLS" : "");
  ...
  public boolean loadServerSettingsFile(String settingsFile)
  {
    ...
    if (fileServerSettings.getProperty("vate.server.auhentication.type") != null)
    {
      found = true;
      try
      {
        tlsAuthentication = fileServerSettings.getProperty("vate.server.auhentication.type", tlsAuthentication ? "TLS" : "").toUpperCase().startsWith("T");
      }
      catch (Throwable e)
      {
...
//      }
      ...
      if (!(userCredentials.size() > 0))
      {
        if (!daemon)
        {
          try
          {
            VTMainConsole.print("VT>Configure a session user account on server?(Y/N, default:Y):");
            String line = VTMainConsole.readLine(true);
...
//          }
        ...
        try
        {
          VTMainConsole.print("VT>Enter session maximum(default:0):");
          line = VTMainConsole.readLine(true);
          if (line == null)
          {
            VTRuntimeExit.exit(0);
      ...
      {
        VTMainConsole.print("VT>Retry configuring server again?(Y/N, default:Y):");
        try
        {
          String line = VTMainConsole.readLine(true);
          if (line == null || line.toUpperCase().startsWith("N"))
...
//        }
    ...
    VTMainConsole.clear();
    VTMainConsole.print("VT>Variable-Terminal " + VTSystem.VT_VERSION + " - Server - (c) " + VTSystem.VT_YEAR + " wknishio@gmail.com\n" + 
  ...
  private void loadFromConnectorToServer()
  {
    if (serverConnector != null)
    {
      ...
      this.tlsAuthentication = serverConnector.getTLSAuthentication();
  ...
  private void saveFromServerToConnector()
  {
    if (serverConnector != null)
    {
      ...
      serverConnector.setTLSAuthentication(tlsAuthentication);
  ...
  public void run()
  {
    ...
    serverConnector = new VTServerConnector(this, new VTBlake3StandardSecureRandom(new SecureRandom()), proxy, managed);
    serverConnector.setPassive(passive);
    serverConnector.setAddress(hostAddress);
    serverConnector.setPort(hostPort);
    serverConnector.setNatPort(natPort);
  ...
  public int getPingLimitMilliseconds()
  {
    ...
    {
      return VTSystem.VT_PING_LIMIT_MILLISECONDS;
    }
  }
  ...
  public int getPingIntervalMilliseconds()
  {
    ...
    {
      return VTSystem.VT_PING_INTERVAL_MILLISECONDS;
    }
  }
  ...
  public int getReconnectTimeoutMilliseconds()
  {
    ...
    {
      return VTSystem.VT_RECONNECT_TIMEOUT_MILLISECONDS;
    }
  }
