...
//  private VTClientConfigurationDialog connectionDialog;
  ...
  "#vate.client.encryption.type      values: SALSA(S)/HC(H)/GRAIN(G)/ZUC(Z)/LEA(L)\r\n" + 
...
//          }
        ...
        String command = VTMainConsole.readLine(true);
        if (command == null)
        {
          VTRuntimeExit.exit(0);
        }
...
//      }
  ...
  public void run()
  {
    ...
    clientConnector = new VTClientConnector(this, new VTBlake3StandardSecureRandom(new SecureRandom()), proxy, managed);
    clientConnector.setActive(active);
    clientConnector.setAddress(hostAddress);
    clientConnector.setPort(hostPort);
    clientConnector.setNatPort(natPort);
